...
my $param_name;                # Parameter set used
...
#-----------------------------+
# Default is to use organism
unless ($param_name) {
    $param_name = $organism_db; 
}
...
#	" --current ".$te_db_dir;
    ...
    system ($te_nest_cmd) unless $do_test;
    ...
    tenest2gff ($do_gff_append, $gff_seqname, 
		$te_nest_result, $gff_out_path,
		$program, $param_name, $feature_type);

# Reference to the new subfunction must go like this
#tenest2gff ($do_append, $seqname, $infile, $outfile, $program, $param_name,
#	    $feature_type);
    # Reference to the old program
...
# NEW CONVERSION SUBFUNCTION FOR GFF3 03/10/2010
sub tenest2gff {
