...
sub trim_gene_at_gap {
    ...
    my $before_gene_obj = &make_trimmed_gene_obj($gene_to_trim, $before_trim_exon_coords_aref, $before_trim_cds_coords_aref, "leftGap:$gap_lend");
    my $after_gene_obj = &make_trimmed_gene_obj($gene_to_trim, $after_trim_exon_coords_aref, $after_trim_cds_coords_aref, "rightGap:$gap_rend");
        ...
        if ($orient eq '+') {
            ...
            if ( ($phase_left + 1) % 3 == $phase_right) {
                ...
                if ( $gap_boundaries_href->{$left_cds_end3 + 1} ) {
                    ...
                    die "Error, cannot figure out which segment neighbors the gap (+).";
                }
